...
async function getArticleDetails(browser, feed) {
...
export default async function says(): Promise<ArticleInterface[]> {
  ...
  for (let i = 0; i < articleLinks.length; i += 1) {
    ...
    article.id = createHash('md5').update(article.link).digest('hex');
    detailsPromise.push(getArticleDetails(browser, article));
  }
