...
describe('Scrape Berita Harian', () => {
  it('should successfully fetch articles', () => {
    mock.onGet('https://www.bharian.com.my/api/articles?sttl=true&page_size=10').reply(200, BeritaHarianFeed);
    new BeritaHarian().scrape()
      .then((result) => expect(result).toEqual(expected));
  });

  it('should failed to fetch articles', () => {
    mock.onGet('https://www.bharian.com.my/api/articles?sttl=true&page_size=10').reply(404);
    new BeritaHarian().scrape()
      .catch((result) => expect(result).toHaveLength(0));
  });
